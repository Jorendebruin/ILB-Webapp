...
export default class Home extends React.Component {
  ...
  toggleInstancesState = async state => {
  ...
  startInstancesState = async () => {
    this.toggleInstancesState(80);
  };
  render() {
    this.filteredInstances = this.state.instances
    ...
    console.log(this.filteredInstances);

    // Put all the instances we are left with in some HTML
    var htmlFormattedInstances = this.filteredInstances.map(instance => {
      return (
        <div
          ...
          <ul className="filters">
            {filterGroup.items.map(filter => {
              ...
              switch (filterGroup.verbose.toLowerCase()) {
                case "locatie":
                  amountLeft = this.filteredInstances.filter(instance => {
      ...
      <div className="homePage row">
        <section className="col-xs-9 row c-scrollable">
          ...
          <h1 className="title col-xs-12">
            Instances ({this.filteredInstances.length})
          </h1>
        ...
        <aside className="c-sidebar col-xs-3">
          ...
          {filters}
          <h1>Action</h1>
          <button onClick={this.startInstancesState}>Start errthang</button>
        </aside>
      </div>
    );
  }
